function [out_batches,SPM_anovas] = OLA_anova(subjects, ...
    beh_data_path,fmri_data_path, firstlevel_data_path)
% OLA_ANOVA generates an analysis of variance (ANOVA) model with
% substance group as factor for the encoding and retrieval phase of the
% OLA task, respectively;
% [out_batches,SPM_anovas] = OLA_ANOVA(subjects,beh_data_path,
...
% Outputs a cell containing the generated and saved matlabbatch and
%   corresponding SPM file for both OLA phases;
...
sequences = {'OLA_E','OLA_R'};
problem_log = {};
out_batches = cell(size(sequences));
SPM_anovas = cell(size(sequences));

events = {'CORSCE-FALSCE', 'CORSCR-FALSCR'};
contrasts = {'con_0004', 'con_0005'};
...
for s=1:numel(sequences)
    ...
    event_name = events(s);
    batch_name = "Anova_" + string(event_name) +".mat";
    cd(firstlevel_data_path);
    output_dir = fullfile("..\OLA_ANOVA\", string(sequence));
    mkdir(output_dir);
    ...
    for i = 1:numel(subjects)
        ...
        confile_path = fullfile(firstlevel_data_path, sequence, ...
            substance, subject_name);
...
%         spm_jobman ('interactive', matlabbatch, '')
        ...
        out_batches{s} = matlabbatch;
        SPM_anovas{s} = load(fullfile(output_dir, "SPM.mat"));
    catch
    fprintf('PROBLEM with OLA ANOVA\n');
    problem_log{end+1} = "1";
    end
    else
         out_batches{s} = load(fullfile(output_dir, batch_name));
         SPM_anovas{s} = load(fullfile(output_dir, "SPM.mat"));
    end
end
writecell(problem_log, fullfile(fileparts(fmri_data_path), "OLA_anova_problem_log.csv"));
end