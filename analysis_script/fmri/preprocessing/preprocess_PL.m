function preprocess_PL(subjects, fmri_data_path)
% PREPROCESS_PL is a preprocessing function for the PL and OLA data (it
% combines all the other preprocessing functions since they do not differ
% in the configs)
% PREPROCESS_PL(subjects, fmri_data_path) takes as the
...
% $Author: A. Kasparbauer, A. Vorreuther $Date: 2022/05/18
arguments
    subjects (1,:) cell;
...
for i =1:numel(subjects)
    ...
    if ~exist(fullfile(sbj_path,string(sequence),'preprocessing_'+string(sequence)+'.mat'), "file")
